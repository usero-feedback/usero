...
`;function vt(){return typeof window>"u"||typeof window.matchMedia!="function"?te:window.matchMedia("(prefers-color-scheme: dark)").matches?te:window.matchMedia("(prefers-color-scheme: light)").matches?he:te}function Ee(t){let n=vt();return t?{...n,...t}:n}var Ze="feedback_user_email";function M(t){return t.replace(/[&<>"']/g,n=>{switch(n){case "&":return "&amp;";case "<":return "&lt;";case ">":return "&gt;";case '"':return "&quot;";case "'":return "&#x27;";default:return n}})}function wt(t,n){let o=t;for(let s of n){if(!s||typeof s!="object")continue;let{metadata:l,...i}=s;o={...o,...i},l&&typeof l=="object"&&(o.metadata={...o.metadata??{},...l});}return o}function St(){if(typeof window>"u")return "";try{return window.localStorage.getItem(Ze)??""}catch{return ""}}function kt(t){try{window.localStorage.setItem(Ze,t);}catch{}}function _t(t){if(typeof document>"u")return {destroy:()=>{},open:()=>{},close:()=>{},update:()=>{},whenReady:()=>Promise.resolve(),identify:()=>{}};let{clientId:n,baseUrl:o}=t;if(!n||n.length<3){let e=new Error("Invalid config. Contact admin.");return t.onError?.(e),{destroy:()=>{},open:()=>{},close:()=>{},update:()=>{},whenReady:()=>Promise.resolve(),identify:()=>{}}}let s=t.position??"right",l=t.theme,i=Ee(l),a=t.title??"Share Feedback",w=t.placeholder??"Tell us what you think... (optional)",A=t.showEmailOption??true,E=t.showScreenshotOption??true,Ue=t.environment,ie=t.metadata,Te=t.onSubmit,se=t.onError,$e=t.onOpen,Ie=t.onClose,V=t.getUser,J=t.user,ae=new ne(o),et={apiUrl:o??K,clientId:n},Y=null,le,de,ce;function C(e){let r=e??null;if(r){if(r.id===Y&&r.traits===le&&r.email===de&&r.displayName===ce)return;Ve(et,r),Y=r.id,le=r.traits,de=r.email,ce=r.displayName;}else Y!==null&&(Je(),Y=null,le=void 0,de=void 0,ce=void 0);}function ue(){if(V)try{C(V()??null);}catch{}}t.user!==void 0?C(t.user):V&&ue();let X=t.plugins??[],fe=new Map,Q=new Map,pe=[];for(let e of X){let r={clientId:n,baseUrl:o??K,logger:Ye(e.name),getStore:()=>fe.get(e.name),setStore:u=>{fe.set(e.name,u);},resolveUser:()=>{W||(J!==void 0?C(J):ue());},getSdkSessionId:()=>je(),getAnonymousId:()=>ke(),getUserId:()=>qe(),getReplayStartMs:()=>Ge(),publishReplayStartMs:u=>Ke(u)};if(Q.set(e.name,r),e.onInit){let u=(async()=>{try{await e.onInit?.(r);}catch(k){r.logger.error("onInit threw",k);}})();pe.push(u);}}let tt=pe.length===0?Promise.resolve():Promise.all(pe).then(()=>{}),b=false,Z=false,D,U="",T=false,O=St(),v=false,h=null,f=[],S=false,y=null,F=3,nt=10*1024*1024,P=document.createElement("div");P.setAttribute("data-usero-widget",""),P.style.cssText="all: initial;",document.body.appendChild(P);let z=P.attachShadow({mode:"open"});function rt(){ue();}function Me(e){try{window.dispatchEvent(new CustomEvent("usero:shadow-update",{detail:{host:P,root:z,reason:e}}));}catch{}}Me("mount");let Fe=document.createElement("style");Fe.textContent=Qe,z.appendChild(Fe);let $=document.createElement("button"),H=document.createElement("div"),c=document.createElement("div");z.appendChild($),z.appendChild(H),z.appendChild(c);function ot(e){h=e,x();}function Pe(){b||(b=true,Z=true,D=void 0,U="",T=false,h=null,f=[],y=null,S=false,ae.ping(),rt(),$e?.(),x(),Me("panel-open"));}async function it(e){if(y=null,!e.type.startsWith("image/")){y="Image files only",R();return}if(e.size>nt){y="Max 10MB",R();return}if(f.length>=F){y=`Max ${F} screenshots`,R();return}S=true,ge(),R();try{let r=await ae.uploadScreenshot(e,n);f=[...f,r];}catch(r){y=r instanceof Error?r.message:"Upload failed";}finally{S=false,ge(),R();}}function Re(e){f=f.filter((r,u)=>u!==e),ge(),R();}function B(){b&&(b=false,Ie?.(),x());}function Le(){return S?'<span class="fb-ups"></span> Uploading...':"\u{1F4F7} Add screenshot"}function st(){let e=f.length>=F,r=S||e;return `
		...
		`}function Ae(){let e=f.length>=F,r=f.map((m,g)=>`
					<div class="fb-sp">
				...
				`}).join(""),k=h?`<div class="fb-msg fb-msg--header ${h.type==="success"?"fb-msg--ok":"fb-msg--err"}">${h.type==="success"?"\u2713":"\u26A0"} ${M(h.text)}</div>`:"",m=E?st():"",g=E?Ae():"",_=A?`
				<div class="fb-email">
					...
					${T?`<input type="email" class="fb-email-inp" data-role="email-input" value="${M(O)}" placeholder="your.email@example.com" aria-label="Email" maxlength="254" autocomplete="email" style="border:1px solid ${i.border};color:${i.text};background-color:${i.background};" />`:""}
				</div>
		...
		`,c.querySelector('form[data-role="form"]')?.addEventListener("submit",d=>{d.preventDefault(),Ce();}),c.querySelector('button[data-role="close"]')?.addEventListener("click",B),c.querySelectorAll("button[data-rating]").forEach(d=>{d.addEventListener("click",()=>{let p=d.dataset.rating;(p==="1"||p==="2"||p==="3"||p==="4")&&(D=Number(p),Z=true,x());});});let j=c.querySelector('textarea[data-role="comment"]');j&&(Z&&(Z=false,requestAnimationFrame(()=>j.focus({preventScroll:true}))),j.addEventListener("input",()=>{if(j.value.length<=1e3){U=j.value;let d=c.querySelector('[data-role="charcount"]');if(d){let p=1e3-U.length;d.textContent=`${p} chars remaining`,d.style.color=p<50?"#dc2626":i.text,d.style.opacity=p<50?"1":"0.6";}}}));let He=c.querySelector('input[data-role="share-email"]');He?.addEventListener("change",()=>{T=He.checked,x();});let be=c.querySelector('input[data-role="email-input"]');be?.addEventListener("input",()=>{be.value.length<=254&&(O=be.value);});let q=c.querySelector('input[data-role="screenshot-input"]');c.querySelector('button[data-role="screenshot-pick"]')?.addEventListener("click",()=>{q?.click();}),q?.addEventListener("change",()=>{let d=q.files?.[0];d&&it(d).finally(()=>{q&&(q.value="");});}),c.querySelectorAll('button[data-role="screenshot-remove"]').forEach(d=>{d.addEventListener("click",()=>{let p=Number(d.dataset.index);Number.isInteger(p)&&Re(p);});});}function x(){at(),lt(),dt();}$.addEventListener("click",()=>{b?B():Pe();}),H.addEventListener("click",()=>{S||v||B();});let De=e=>{if(b){if(e.key==="Escape"){if(S||v)return;B();}e.key==="Enter"&&(e.metaKey||e.ctrlKey)&&(e.preventDefault(),Ce());}};document.addEventListener("keydown",De);let L=null,N=null;function Oe(){L&&N&&L.removeEventListener("change",N),L=null,N=null;}function ze(){L||typeof window>"u"||typeof window.matchMedia!="function"||(L=window.matchMedia("(prefers-color-scheme: dark)"),N=()=>{l===void 0&&(i=Ee(void 0),x());},L.addEventListener("change",N));}l===void 0&&ze(),x();let W=false;return {destroy:()=>{if(!W){W=true,document.removeEventListener("keydown",De),Oe();for(let e of X){if(!e.onDestroy)continue;let r=Q.get(e.name);if(r)try{e.onDestroy(r);}catch(u){r.logger.error("onDestroy threw",u);}}fe.clear(),Q.clear(),P.remove();}},open:Pe,close:B,whenReady:()=>tt,identify:e=>{W||(J=e,C(e));},update:e=>{if(W)return;let r=false;e.position!==void 0&&e.position!==s&&(s=e.position,r=true),"theme"in e&&(l=e.theme,i=Ee(l),l===void 0?ze():Oe(),r=true),e.title!==void 0&&e.title!==a&&(a=e.title,r=true),e.placeholder!==void 0&&e.placeholder!==w&&(w=e.placeholder,r=true),e.showEmailOption!==void 0&&e.showEmailOption!==A&&(A=e.showEmailOption,r=true),e.showScreenshotOption!==void 0&&e.showScreenshotOption!==E&&(E=e.showScreenshotOption,r=true),"environment"in e&&(Ue=e.environment),"metadata"in e&&(ie=e.metadata),"onSubmit"in e&&(Te=e.onSubmit),"onError"in e&&(se=e.onError),"onOpen"in e&&($e=e.onOpen),"onClose"in e&&(Ie=e.onClose),"getUser"in e&&(V=e.getUser),"user"in e&&(J=e.user,C(e.user)),r&&x();}}}exports.DARK_THEME=te;exports.DEFAULT_THEME=he;exports.initUseroFeedbackWidget=_t;exports.mergePluginPatches=wt;exports.mergeTheme=ut;exports.resolveTheme=Ee;return exports;})({});//# sourceMappingURL=usero.iife.js.map
