...
Minor. New headless core for consumers who want to build their own feedback UI on the widget's logic. `createUseroFeedback(options)` from the new `@usero/sdk/headless` subpath returns a controller with `submit` (validates, captures page context, runs the plugin pipeline, POSTs; resolves `{ success: false, error }` instead of throwing), `uploadScreenshot`, `identify` (deduped, `null` rotates the anonymousId on logout), `whenReady` (settles after every plugin `onInit`), `notifyShadowMount` (re-snapshot signal for custom UIs rendered inside a ShadowRoot), and `destroy`. There is deliberately no `open`/`close`/`isOpen`: UI state belongs to the consumer. Because the plugin pipeline runs on every `submit()`, passing `sessionReplay()` into `plugins` links custom-UI submissions to the running recording (sessionReplayId plus replayOffsetMs) exactly as the widget does. React consumers get `useUseroFeedback(options)` from `@usero/sdk/headless/react` (SSR-safe, StrictMode-safe, destroys on unmount, reactive `user` option). All payload, response, user, and plugin types are exported so an integration can be fully typed. Internally the widget's identity resolution, plugin runtime, and submit pipeline moved to a shared `core.ts` consumed by both `@usero/sdk` and the headless entry; widget behaviour is unchanged and the full existing test suite passes as-is. The headless entry ships no widget CSS, no React, and no rrweb (3.2KB gzipped minified); existing base bundles are unaffected.

## 1.2.0

Minor. Session replay gains a standalone, widget-free mode. The new canonical subpath `@usero/sdk/replay` exports the same `sessionReplay()` factory, now dual-mode: pass the instance to the widget's `plugins` array exactly as before, or call `.start()` to record without mounting the widget (no launcher button, no DOM footprint). Standalone mode needs only `clientId`, plus optional `user`/`getUser` for identity (re-resolved at session start and chunk boundaries) and `apiUrl` to override the default https://usero.io host. `.start()` is idempotent and SSR-safe; `.stop()` flushes buffered events and finalises the session, and a later `.start()` begins a new session under the same per-tab sdkSessionId. At most one recording runs per page, coordinated through a globalThis slot, so a widget mounting while a standalone recording is live links its feedback submissions to the running recording (sessionReplayId plus replayOffsetMs) instead of double-recording, and a widget unmount never kills a recording it merely adopted. React consumers get `useSessionReplay(options)` from the new `@usero/sdk/replay/react` subpath, which is StrictMode-safe and page-scoped (recording survives the component unmounting on client-side route changes). The legacy `@usero/sdk/plugins/session-replay` subpath keeps working as a re-export of the same module, so existing consumers need no changes. Base bundles (`@usero/sdk`, `@usero/sdk/react`) are unchanged and rrweb stays a lazy runtime chunk.
